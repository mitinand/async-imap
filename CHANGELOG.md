...
- `fetch()`, `uid_fetch()`, `store()` and `uid_store()` report a NO or BAD completion, and a connection closed before the completion, as an error after the responses received before it; previously the stream ended as if the command had succeeded.
...
- Trace logging of sent commands shows only the command name and size; arguments and continuation data such as passwords and tokens are no longer logged.
- `capabilities()` returns an error when the server rejects `CAPABILITY` with `NO` or `BAD` instead of an empty capability set.
...
[0.11.3]: https://github.com/chatmail/async-imap/compare/v0.11.2...v0.11.3
